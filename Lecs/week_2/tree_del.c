...
void
tree_delete(Tree, int);

Tree
tree_parent(Tree, int);
...
void
tree_delete(Tree T, int it)
{
	assert(T != NULL);
	// If doesn't contain it then simply return
	if(TreeContains == false)	return;
	// Delete root and only node
	if(left(T) == NULL && right(T) == NULL && data(T) == it)	
	{		
		free(T);
		return;
	}
	...
	// Recursively delete
		...
		{	
			Tree par = T;		
			left(T) = tree_delete(left(T), it);
		}		
		else if(it > data(T))	
		{
			Tree par = T;		
			right(T) = tree_delete(right(T), it);
		}

	// Case of one child node
		else if(T->left == NULL)
		{
			if(it < data(parent))
			{
	...
	// Case of one child node
		...
		{
			// Create a function to keep track of parent
			if(it < data(parent))
			{
				left(parent) = T->left;
				free(T);
	...
	// Join the root node
		...
		else
		{
			if(par == NULL)	
			{					
				Tree new = tree_join(T->left, T->right);
				free(T);
			}
			else
			{
				//iterate to left of parent and call tree join on 
			}
		}
...
Tree
tree_parent(Tree T, int it)
{
	if(data(left(T)) || data(right(T)) == it)	return T;
			
	tree_parent(T->left, it);
	tree_parent(T->left, it);	
}
